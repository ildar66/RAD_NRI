...
public class PeopleMapper extends JDBCMapper implements com.nri.mapping.PeopleMapper {
	...
	protected static final String findByNameSqlString =
		"SELECT man, firstname, lastname, middlename, passportser, passportdate, passportwhom, isactive, tabnum FROM "
			+ ApplProperties.getDatasourceSchema()
			+ ".PEOPLE WHERE LOWER(lastname) matches LOWER(?)";
	...
	protected People activate(ResultSet rs) throws SQLException {
		...
		people.setFirstName(rs.getString(2).trim());
		people.setLastName(rs.getString(3).trim());
		people.setMiddleName(rs.getString(4).trim());
	...
	public Page findByName(String name, Boolean isActive, int start, int count, String orderBy) throws MappingException {
		...
		try {
			...
			PreparedStatement ps = conn.prepareStatement(sb.toString(), ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			ps.setString(1, name);
			if (isActive != null) {
