...
public class OIDvalueBeanFunctionSet_9352f82e extends com.ibm.ws.rsadapter.cci.WSResourceAdapterBase implements com.ibm.websphere.rsadapter.DataAccessFunctionSet {
	...
	public javax.resource.cci.Record FindByPrimaryKey(IndexedRecord inputRecord, Object connection, WSInteractionSpec interactionSpec) throws DataStoreAdapterException {
		...
		try {
			pstmt = prepareStatement(connection,"SELECT T1.VALUE, T1.ID FROM INFORMIX.SEQUENCES  T1 WHERE T1.ID = ?");
	...
	public javax.resource.cci.Record FindByPrimaryKeyForUpdate(IndexedRecord inputRecord, Object connection, WSInteractionSpec interactionSpec) throws DataStoreAdapterException {
		...
		ResultSet result=null;
		try {
			pstmt = prepareStatement(connection,"SELECT T1.VALUE, T1.ID FROM INFORMIX.SEQUENCES  T1 WHERE T1.ID = ? FOR UPDATE ");
